...
<script>
	$(document).ready(function(){
		...
		$('.menu_item').click(function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					$('#show_items').append('<p>' + item_name +'<span class="float-right">Rs'+item_price+'</p>');
		...
		$('#place').click(function(){
			window.location.href="http://localhost:10080/zomato/order_details.php?order_id="+order_id;
		})
	})
</script>
<body>
	...
	<nav class="navbar" style="background: linear-gradient(to top, #b31217, #e52d27);">
		...
		<h5 class="float-right text-light mt-1">Hi, <?php echo $_SESSION['name']?> </h5>
	</nav>
										...
										<div class="card-body">
											...
											echo'
												<div class="col-md-7">
													...
													<p>Rs <span id="menu_item_price'.$row['id'].'"> '.$row['price'].' </span> </p>
													<small> '.$row['desc'].' </small>
												</div>
												<div class="col-md-2">
													<button data-id='.$row['id'].' class="btn btn-danger btn-sm menu_item"style="background: linear-gradient(to top, #b31217, #e52d27);">Add &nbsp +</button>
												</div>
